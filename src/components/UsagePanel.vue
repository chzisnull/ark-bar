...
<template>
  <div class="flex flex-col h-full select-none text-slate-200 bg-[#0e131f]/95">
    ...
    <div class="p-3.5 flex-1 overflow-y-auto space-y-3">
      ...
      <div v-if="currentUsage?.is_connected" class="space-y-3">
        ...
        <div class="pt-1">
          <button
            ...
            class="w-full py-2 bg-gradient-to-r from-indigo-500/10 to-violet-500/10 hover:from-indigo-500/20 hover:to-violet-500/20 border border-indigo-500/20 hover:border-indigo-500/40 rounded-xl text-xs text-indigo-300 font-medium flex items-center justify-center space-x-1.5 transition duration-150"
          >
      ...
      <div v-else class="p-4 bg-[#131a2a]/80 border border-slate-800/80 rounded-xl text-center space-y-3.5 my-2">
        ...
        <div class="pt-1 flex flex-col gap-2">
          <button
            @click="$emit('open-settings')"
            class="w-full py-2 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white rounded-xl text-xs font-medium flex items-center justify-center gap-1.5 transition shadow-md shadow-indigo-500/20"
          >
            <KeyRound class="w-3.5 h-3.5" />
            <span>前往设置填入令牌 / 授权</span>
          </button>
          <button
            @click="$emit('refresh')"
            :disabled="isRefreshing"
            class="w-full py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-medium flex items-center justify-center gap-1 transition"
          >
